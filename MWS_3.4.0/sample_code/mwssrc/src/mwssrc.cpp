// 日期      版本       維護人員    修改原因.
// 20240206  v01.00.00  吳青華      新程式開發.
//
// 系統名稱: 測試子系統.
// 程式代號: mwssrc.
// 程式名稱: source sample code.
// 安全管制: 否.
// 開發語言: C++.
// 範例程式: 無.
//
// 程式功能: mwssrc sample code.
//
// 執行時機: 沒有限制.
// RERUN   : 可.
//
...
// ARGV[2]   src cfg's section namee      src 設定在設定檔中的 section.
// ARGV[>3]  以下為選擇性參數:
...
// src 設定在設定檔中的 section name.
string g_src_section_name("");
// MWS log 檔位置.
...
int src_callback(mws_event_t* event_ptr,
  ...
  if (g_msgs_per_second > 0)
  {
    ...
    while (1)
    {
          ...
          (is_ready_to_send == true))
      {
        ...
        string msg_str = g_src_ptr->get_topic_name() + ":" + to_string(++msg_number);
        ...
        int send_rtv = g_src_ptr->mws_hf_src_send((char*)&msg[0], g_size_of_a_msg, msg_number);
        if (send_rtv < 0)
        {
...
{
  // Begin: mws 初始化.
  int rtv = mws_init(string("mwssrc"),
                     g_mws_cfg_file_path,
                     g_mws_log_file_path,
                     1);
  if (rtv == (-1))
  ...
  // Begin: 建立 src.
  g_src_ptr = new mws_src_t(g_src_section_name,
                            g_ctx_ptr,
                            g_evq_ptr,
                            src_callback);
  returned_value = g_src_ptr->mws_get_object_status();
  if (returned_value != MWS_NO_ERROR)
  {
    string log_body = "create mws_src 失敗";
    stringstream log_body_ss("");
    log_body_ss << "error: " << returned_value;
    log_body += log_body_ss.str();
    cerr << log_body << endl;
...
// 功能: 處理 source 的 event (內容由 AP 的程式設計師依照業務撰寫).
// 回傳值 0: 表示成功.
                 ...
                 size_t custom_data_length)
{
  switch (event_ptr->event_type)
  {
    ...
    case MWS_SRC_EVENT_DISCONNECT:
    {
      ...
      g_received_bytes_during_this_sec = 0;
      pthread_mutex_unlock(&g_mutex);
      printf("[MWS_SRC_EVENT_DISCONNECT]===============\n");
      cout << "fd: " << event_ptr->fd << "\n";
      cout << "topic_name: " << event_ptr->topic_name << "\n";
      cout << "src_addr ( " << event_ptr->src_addr.str_ip << ": " << event_ptr->src_addr.str_port << " )\n";
      cout << "rcv_addr ( " << event_ptr->rcv_addr.str_ip << ": " << event_ptr->rcv_addr.str_port << " )" << endl;
    ...
    {
      printf("[MWS_SRC_UNRECOVERABLE_LOSS]====\n");
      cout << "fd: " << event_ptr->fd << "\n";
      cout << "topic_name: " << event_ptr->topic_name << "\n";
...
{
  string str("");
  str = "Usage: mwssrc CFG_FILE_PATH SRC'S_SECTION_NAME";
  cout << str << "\n";
...
bool check_argument()
{
  ...
  if (g_src_section_name.size() <= 0)
  {
    cout << "SRC'S_SECTION_NAME is required." << endl;
    rtv = false;
  }
...
void debug_show_argument()
{
  ...
  cout << "g_src_section_name: " << g_src_section_name << endl;
