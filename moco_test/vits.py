...
class ConvStem(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # assert patch_size == 10, 'ConvStem only supports patch size of 16'
        ...
        stem = []
        input_dim, output_dim = in_chans, embed_dim // 4
        ...
        #     input_dim = output_dim
        #     output_dim *= 2
        # stem.append(nn.Conv2d(input_dim, output_dim, kernel_size=3, stride=2, padding=2, bias=False))
        # stem.append(nn.BatchNorm2d(output_dim))
        # stem.append(nn.ReLU(inplace=True))
        # input_dim = output_dim
        # output_dim *= 2
        ...
        # output_dim = 768
        stem.append(
            nn.Conv2d(input_dim, output_dim, kernel_size=4, stride=1, padding=0, bias=False)
        )
        stem.append(nn.BatchNorm2d(output_dim))
        stem.append(nn.ReLU(inplace=True))
        input_dim = output_dim
...
def vit_conv_small(**kwargs):
    ...
    model.head = nn.Linear(32, 128)
    # embed_layer=ConvStem, in_chans=4,  **kwargs)
    model.default_cfg = _cfg()
    return model
