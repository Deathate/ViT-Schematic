...
class DoubleLineFormalDataset(Datasetbehaviour):
    ...
    def __create(self, total_output, line_num):
        ...
        def create_img_v2(rng):
            ...
            return (plotly_to_array(fig), starting_point[0]), (start_all, starting_point)
...
def transform(x):
    return image_process(x[0]).cuda(), torch.tensor([x[1]]).float().cuda()
...
def tt(y_hat, y):
    all_point = y_hat[:, :4].reshape(-1, 2, 2)
    step1 = Hungarian_Order(all_point, y[:, :4].reshape(-1, 2, 2)).flatten(1)
    step2 = y_hat[:, 4:].reshape(-1, 2)
    x = torch.cat([step1, step2], dim=1)

    return x


model.fit(m, nn.MSELoss(), optim.AdamW(m.parameters(), lr=1e-3),
          2000, target_transform=tt)
# %%
get_local.activate()
