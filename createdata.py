...
class MessyDataset(Datasetbehaviour):
    def __init__(self, size):
        self.postive = [cv.imread(str(x), cv.IMREAD_UNCHANGED) for x in list(
            Path("data_cleaning_example/positive").glob("*/*"))]
        self.negative = [cv.imread(str(x), cv.IMREAD_UNCHANGED) for x in list(
            Path("data_cleaning_example/negative").glob("*/*"))]
    ...
    def __create(self):
        ...
        def add_dottedline(img, start_point, end_point):
            ...
            for i in range(num_segments):
                start_dot = (int(start_point[0] + i * x_increment),
                             int(start_point[1] + i * y_increment))
                end_dot = (int(start_point[0] + (i + 0.5) * x_increment),
