...
def analyze_connection(
    ...
    load_model(config.get_best_model_path(dataset_config, full_size))
    ...
    with HiddenPrints(disable=debug), HiddenPlots(disable=debug):
        ...
        if not full_size:
            ...
            for i in range(len(dataset)):
                ...
                if image.mean() < 254.5 and mask.max() == 255:
                    lines = []
                    lines = predict_results[i]
                    # remove point that is on empty space
                            ...
                            if (
                                ...
                                for point in line:
                                    ...
                                    radius_pixel = image_bk_gray[
                                        ...
                                        max(start - radius, 0) : min(
                                            start + radius, slice_size - 1
                                        ),
                                    ]
            ...
            img, _, half_pad = image_preprocess(input_img)
            img = cv2.resize(img, (224, 224), cv2.INTER_LINEAR_EXACT)
            dataset = TestWindowed(img, -1, -1)
            predict_results = predict_mask_imgs(dataset, min_line_length)[0]
            if debug:
                print("model prediction figure")
                # plot_images(img, debug_img_width)
                plot_images(draw_line(img, predict_results), debug_img_width)
            ratio = half_pad / input_img_shape[0]
            predict_results[:, :, 1] = (predict_results[:, :, 1] - ratio) * (1 / (1 - 2 * ratio))
            ratio = half_pad / input_img_shape[1]
            predict_results[:, :, 0] = (predict_results[:, :, 0] - ratio) * (1 / (1 - 2 * ratio))
            global_connection = build_connection(
                predict_results,
                norm1,
                similar_threshold=0,
                threshold=global_threshold,
...
if __name__ == "__main__":
    ...
    # test_list = ["circuit2477.jpg"]
    # test_list = ["000001.jpg"]
    if config.REAL_DATA:
        data_config = config.DatasetConfig.REAL
    else:
        data_config = config.DatasetConfig.CC
    img_name = Path(config.TEST_DATASET_PATH + "/images/" + test_list[5])
